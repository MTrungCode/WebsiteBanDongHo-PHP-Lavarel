...
class AdminProductController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($trademark = $request->trademark) $products->where('pro_trademark', $trademark);
    ...
    public function update(AdminRequestProduct $request, $id)
    {
        ...
        $data               = $request->except('_token', 'pro_avatar', 'attributes');
        $data['pro_slug']     = Str::slug($request->pro_name);
    ...
    protected function syncAttribute($attributes, $idProduct)
    {
        if (!empty($attributes)) {
            ...
            if (!empty($datas)) {
                \DB::table('products_attributes')->where('pa_product_id', $idProduct)->delete($datas);
